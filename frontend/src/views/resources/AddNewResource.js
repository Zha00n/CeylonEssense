...
const AddNewResource = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/r/add-resource`, data, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        setErrorMessage('');
        alertTypes.success('Product added successfully!');
      ...
      if (error.response && error.response.status === 401) {
        ...
        if (refreshed) {
          ...
          try {
            const retryResponse = await axios.post(`${API_URL}/r/add-resource`, data, {
              headers: {
            ...
            alertTypes.error('Failed to add Product Resource!');
