...
const UpdateEvent = ({ eventId, handleModal , handleUpdate }) => {
  ...
  const fetchEventData = async () => {
    if (!eventId) {
      ...
      return; // Prevent the request if eventId is not available
    }
    
    try {
      const response = await apiRequest(`${API_URL}/ev/get/${eventId}`,);
  ...
  try {
    const response = await apiRequest(`${API_URL}/ev/update-event/${eventId}`, {
      method: 'PUT',
      body: data,
    });
